...
func TestLoggerWritesJSONLinesTaggedWithService(t *testing.T) {
	...
	logger := observability.NewLoggerTo(&buf, "video-processor-worker")
	...
	assert.Equal(t, "video-processor-worker", line["service"])
...
func TestSetupTracingWithoutEndpointIsANoOpThatStillShutsDown(t *testing.T) {
	...
	shutdown, err := observability.SetupTracing(t.Context(), "video-processor-worker")
